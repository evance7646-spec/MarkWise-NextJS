  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">Optimizer</h1>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm">
        <h2 className="font-semibold text-slate-700 dark:text-slate-200 mb-4">Search Parameters</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          <div>
            ...
            <input
              ...
              className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm dark:bg-slate-700 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            />
          </div>
          <div>
        ...
        <button
          onClick={search}
          disabled={loading}
          className="mt-4 inline-flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-60 text-white text-sm font-medium rounded-xl transition-colors shadow-sm"
        >
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400">{room.name}</p>
                  </div>
...
function UnderutilizedRooms({ auth }: { auth: { token: string; institutionId: string } }) {
  ...
  return (
    <div className="space-y-4">
      {/* Controls */}
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4 shadow-sm flex flex-wrap items-center gap-4">
        <div className="flex items-center gap-2">
          <label className="text-sm text-slate-600 dark:text-slate-300">Utilization below</label>
          <input
            type="number"
            min={1}
            max={100}
            ...
            className="w-20 border border-slate-300 dark:border-slate-600 rounded-lg px-2 py-1.5 text-sm dark:bg-slate-700 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
          />
          ...
          {selected.size > 0 && (
            <>
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-rose-600 hover:bg-rose-700 disabled:opacity-60 text-white text-xs font-medium rounded-lg transition-colors"
              >
              ...
              <button
                ...
                disabled={bulkLoading}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60 text-white text-xs font-medium rounded-lg transition-colors"
              >
                Set Free
              </button>
            </>
          )}
          <button onClick={load} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-500 transition-colors">
            ...
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-700">
                <th className="px-4 py-3 text-left">
                  <button onClick={toggleAll} className="text-slate-500 hover:text-indigo-600 transition-colors">
                ...
                <th className="px-4 py-3 text-left font-medium text-slate-600 dark:text-slate-300">Room</th>
                <th className="px-4 py-3 text-left font-medium text-slate-600 dark:text-slate-300">Type</th>
                <th className="px-4 py-3 text-right font-medium text-slate-600 dark:text-slate-300">Capacity</th>
                <th className="px-4 py-3 text-right font-medium text-slate-600 dark:text-slate-300">Bookings</th>
                <th className="px-4 py-3 text-right font-medium text-slate-600 dark:text-slate-300">Hours Booked</th>
                <th className="px-4 py-3 text-left font-medium text-slate-600 dark:text-slate-300">Utilization</th>
              </tr>
            </thead>
                ...
                filtered.map((room) => (
                  ...
                  >
                    <td className="px-4 py-3">
                      <button onClick={() => toggle(room.roomId)} className="text-slate-400 hover:text-indigo-600 transition-colors">
                    ...
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{room.type}</td>
                    <td className="px-4 py-3 text-right text-slate-600 dark:text-slate-300">{room.capacity}</td>
                    <td className="px-4 py-3 text-right text-slate-600 dark:text-slate-300">{room.bookingCount}</td>
                    <td className="px-4 py-3 text-right text-slate-600 dark:text-slate-300">{room.hoursBooked.toFixed(1)}h</td>
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4 shadow-sm flex items-center gap-4">
        <label className="text-sm text-slate-600 dark:text-slate-300 whitespace-nowrap">Select Room</label>
        {roomsLoading ? (
          ...
          <select
            ...
            className="border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm dark:bg-slate-700 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
          >
            {rooms.map((r) => (
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4 shadow-sm overflow-x-auto">
          <p className="text-xs text-slate-400 mb-3">Based on bookings in the past 14 days and upcoming 14 days</p>
          <table className="w-full" style={{ tableLayout: "fixed" }}>
            <thead>
              <tr>
                <th className="w-24 text-xs text-slate-400 font-normal text-left pb-2">Day</th>
                {HOURS.map((h) => (
                  <th key={h} className="text-[10px] text-slate-400 font-normal text-center pb-2 px-0.5">
                    {fmt12(h)}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {DAYS.map((day) => (
                <tr key={day}>
                  <td className="text-xs text-slate-500 dark:text-slate-400 font-medium pr-2 py-0.5 whitespace-nowrap">
                    {day.slice(0, 3)}
                  </td>
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-2xl p-8 text-center">
          ...
          <p className="font-medium text-emerald-700 dark:text-emerald-400">No conflicts found</p>
            ...
            >
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-slate-500 dark:text-slate-400">{conflict.day}</span>
                </div>
                {expanded.has(i) ? (
              ...
              {expanded.has(i) && (
                <div className="border-t border-slate-100 dark:border-slate-700 px-5 py-4 space-y-3">
                  ...
                  {conflict.entries.map((entry, j) => (
                    ...
                    >
                      <span className="text-xs font-bold text-slate-400 mt-0.5">#{j + 1}</span>
                      <div className="space-y-0.5">
                        ...
                        {entry.course && (
                          <p className="text-xs text-slate-400">{entry.course.name}</p>
                        )}
                      </div>
                    </div>
                  ))}
...
function CapacityWasteReport({ auth }: { auth: { token: string; institutionId: string } }) {
  ...
  return (
    <div className="space-y-4">
      {/* Controls */}
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4 shadow-sm flex flex-wrap items-center gap-5">
        <div className="flex items-center gap-2">
          <label className="text-sm text-slate-600 dark:text-slate-300 whitespace-nowrap">Utilization &lt;</label>
          <input
            type="number"
            min={1}
            max={100}
            ...
            className="w-16 border border-slate-300 dark:border-slate-600 rounded-lg px-2 py-1.5 text-sm dark:bg-slate-700 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
          />
          <span className="text-sm text-slate-500">%</span>
        </div>
        <div className="flex items-center gap-2">
          <label className="text-sm text-slate-600 dark:text-slate-300 whitespace-nowrap">Capacity ≥</label>
          <input
            type="number"
            min={1}
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          <select
            ...
            className="border border-slate-300 dark:border-slate-600 rounded-lg px-2 py-1.5 text-sm dark:bg-slate-700 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
          >
      ...
      {!loading && filtered.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl p-4">
            <p className="text-xs text-amber-600 dark:text-amber-400 font-medium uppercase tracking-wide">Rooms Below Threshold</p>
            <p className="text-3xl font-bold text-amber-700 dark:text-amber-300 mt-1">{filtered.length}</p>
          </div>
          <div className="bg-rose-50 dark:bg-rose-900/20 border border-rose-200 dark:border-rose-800 rounded-2xl p-4">
            <p className="text-xs text-rose-600 dark:text-rose-400 font-medium uppercase tracking-wide">Estimated Wasted Seat-Cycles</p>
            <p className="text-3xl font-bold text-rose-700 dark:text-rose-300 mt-1">{totalWastedSeats.toLocaleString()}</p>
          </div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-2xl p-8 text-center">
          <div className="text-3xl mb-2">✅</div>
          <p className="font-medium text-emerald-700 dark:text-emerald-400">No wastage detected</p>
        ...
        <div className="space-y-3">
          {filtered.map((room) => {
            ...
            return (
              <div
                key={room.roomId}
                className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4 shadow-sm flex flex-col sm:flex-row sm:items-center gap-4"
              >
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-slate-800 dark:text-slate-100">
                    {room.roomCode}
                    <span className="ml-2 text-xs font-normal text-slate-400">{room.name}</span>
                  </p>
                ...
                <div className="flex items-center gap-6 shrink-0">
                  ...
                  <div className="text-center">
                    <p className="text-xs text-slate-400">Waste Score</p>
