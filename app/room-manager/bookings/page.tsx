...
const STATUS_CFG: Record<BookingStatus, { label: string; bg: string; text: string; dot: string }> = {
  reserved:  { label: "Reserved",  bg: "bg-amber-100 dark:bg-amber-900/30",   text: "text-amber-700 dark:text-amber-300",   dot: "bg-amber-500"  },
  occupied:  { label: "Occupied",  bg: "bg-rose-100 dark:bg-rose-900/30",     text: "text-rose-700 dark:text-rose-300",     dot: "bg-rose-500"   },
  cancelled: { label: "Cancelled", bg: "bg-slate-100 dark:bg-slate-800/50",   text: "text-slate-600 dark:text-slate-300",   dot: "bg-slate-400"  },
...
export default function BookingsPage() {
  ...
  useEffect(() => {
    ...
    if (!payload?.institutionId) { router.push("/room-manager/login"); return; }
    setToken(tok);
  ...
  }, [institutionId, token, fromDate, toDate]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-indigo-800 dark:text-indigo-300">Bookings</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {TABS.map(t => {
          ...
          const colors: Record<string, string> = {
            ...
            cancelled: "from-slate-50 to-slate-100 dark:from-slate-900/40 dark:to-slate-800/30 border-slate-200 dark:border-slate-700",
          };
          const textColors: Record<string, string> = {
            all: "text-indigo-800 dark:text-indigo-300", active: "text-amber-700 dark:text-amber-300",
            upcoming: "text-blue-700 dark:text-blue-300", completed: "text-emerald-700 dark:text-emerald-300",
      ...
      <div className="bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl p-4 flex flex-col sm:flex-row items-start sm:items-center gap-3">
        ...
        <div className="relative flex-1 min-w-0">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 text-sm rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-400"
          />
          {search && (
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <input type="date" value={fromDate} onChange={e => setFromDate(e.target.value)}
            className="text-sm border border-slate-200 dark:border-slate-600 rounded-lg px-2 py-2 bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-400"
          />
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-2 text-sm rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-600 flex-shrink-0"
        >
      ...
      <div className="bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center gap-3">
            ...
            <button onClick={fetchBookings} className="px-4 py-2 text-sm bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">
              Try again
            </button>
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center gap-3">
            ...
            <p className="text-sm text-slate-400 dark:text-slate-500">Try adjusting the date range or clearing the search</p>
          </div>
        ) : (
          <>
            {/* Table header */}
            <div className="hidden sm:grid grid-cols-[1fr_1fr_1fr_auto_auto] gap-4 px-4 py-2 bg-slate-50 dark:bg-slate-900/40 border-b border-slate-100 dark:border-slate-700">
              <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Room</span>
              <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">When</span>
              <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Unit / Lecturer</span>
              <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Status</span>
              <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Action</span>
            </div>
              ...
              {pageBookings.map(booking => {
                ...
                return (
                  <motion.div key={booking.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                    {/* Main row */}
                    <div
                      className="px-4 py-3 grid grid-cols-1 sm:grid-cols-[1fr_1fr_1fr_auto_auto] gap-2 sm:gap-4 items-center hover:bg-slate-50/50 dark:hover:bg-slate-700/20 cursor-pointer"
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-slate-400 capitalize">{booking.room?.type ?? ""}</div>
                        </div>
                      </div>
                      {/* When */}
                      <div>
                        <div className="text-sm text-slate-700 dark:text-slate-300 font-medium">{formatDate(booking.startAt)}</div>
                        <div className="text-xs text-slate-400 flex items-center gap-1">
                          <Clock className="w-3 h-3" />
                          {formatTime(booking.startAt)} – {formatTime(booking.endAt)}
                          <span className="ml-1 text-indigo-500 dark:text-indigo-400">({formatDuration(booking.startAt, booking.endAt)})</span>
                        </div>
                      </div>
                      {/* Unit / Lecturer */}
                      <div>
                        <div className="text-sm text-slate-700 dark:text-slate-300">{booking.unitCode ?? booking.unit?.code ?? "—"}</div>
                        <div className="text-xs text-slate-400">{booking.lecturer?.fullName ?? booking.lecturer?.email ?? "—"}</div>
                      </div>
                      {/* Status */}
                        ...
                        >
                          <div className="px-4 pb-4 pt-1 bg-slate-50/60 dark:bg-slate-900/30 grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                            {[
                              ...
                              <div key={item.label} className="bg-white dark:bg-slate-800 rounded-lg px-3 py-2">
                                <div className="text-slate-400 dark:text-slate-500 text-xs">{item.label}</div>
                                <div className="text-slate-700 dark:text-slate-300 font-mono mt-0.5 break-all">{item.value}</div>
                              </div>
                            ))}
                          </div>
                        </motion.div>
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="p-2 rounded-lg border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 disabled:opacity-40 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors"
            >
            ...
            {Array.from({ length: Math.min(totalPages, 7) }).map((_, i) => {
              ...
              return (
                <button
                  key={p}
                  onClick={() => setPage(p)}
                  className={`w-8 h-8 rounded-lg text-sm font-medium transition-colors ${p === page ? "bg-indigo-600 text-white" : "border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700"}`}
                >
                  {p}
                </button>
              );
          ...
          >
            ...
            >
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100">Cancel Booking</h2>
                  <p className="text-sm text-slate-500 dark:text-slate-400">This action will notify the assigned lecturer.</p>
                </div>
              </div>
              <div className="bg-slate-50 dark:bg-slate-800 rounded-xl p-4 mb-5 space-y-2 text-sm">
                <div className="flex items-center gap-2 text-slate-700 dark:text-slate-300">
                  ...
                  <span className="text-slate-400">{cancelTarget.room?.name}</span>
                </div>
                <div className="flex items-center gap-2 text-slate-600 dark:text-slate-400">
                  <Clock className="w-4 h-4 text-slate-400" />
                  {formatDate(cancelTarget.startAt)}, {formatTime(cancelTarget.startAt)} – {formatTime(cancelTarget.endAt)}
                  <span className="text-indigo-500 dark:text-indigo-400">({formatDuration(cancelTarget.startAt, cancelTarget.endAt)})</span>
                </div>
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 px-4 py-2 text-sm font-medium rounded-xl border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 disabled:opacity-50 transition-colors"
                >
                  Go Back
                </button>
                <button
                  ...
                  className="flex-1 px-4 py-2 text-sm font-bold rounded-xl bg-rose-600 text-white hover:bg-rose-700 disabled:opacity-50 transition-colors flex items-center justify-center gap-2"
                >
