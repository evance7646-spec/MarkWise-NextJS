    ...
    const boot = async () => {
      ...
      try {
        ...
        if (!deptId) {
          ...
          setTimeout(() => router.push("/department/login"), 2000);
          return;
        }
        if (!alive) return;
              ...
              <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-xl shadow-sm p-5 border border-white/50 dark:border-slate-700/50">
                ...
                {courses.length === 0 && (
                  <div className="mb-4 flex items-start gap-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 px-4 py-3 text-sm text-amber-700 dark:text-amber-300">
                    ...
                    <span>No courses found for this department. Please set up the curriculum first in the <a href="/department/dashboard/curriculum" className="underline font-medium hover:text-amber-900 dark:hover:text-amber-100">Curriculum page</a>.</span>
                  </div>
                )}
