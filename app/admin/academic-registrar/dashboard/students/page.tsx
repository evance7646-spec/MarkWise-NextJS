...
const inp = "w-full rounded-xl border border-gray-200 bg-white px-3 py-2.5 text-sm text-gray-800 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500";
const lbl = "block text-xs font-medium text-gray-500 mb-1.5";
const PAGE = 25;

interface Student {
  id: string; name: string; admissionNumber: string;
  email: string; courseId: string; departmentId: string;
...
function AddStudentModal({ depts, courses, onClose, onSaved }: {
  ...
  const [form, setForm] = useState({ name: "", admissionNumber: "", email: "", courseId: "", departmentId: "" });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <motion.div initial={{ scale: 0.93, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
        ...
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-semibold text-gray-800 flex items-center gap-2"><UserPlus className="h-4 w-4 text-emerald-600" /> Admit Student</h2>
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button onClick={save} disabled={saving} className="flex-1 rounded-xl bg-emerald-600 py-2.5 text-sm font-medium text-gray-900 hover:bg-emerald-500 disabled:opacity-60">
              {saving ? "Saving…" : "Admit"}
            </button>
          </div>
        </div>
...
function BulkImportModal({ institutionId, depts, courses, onClose, onSaved }: {
  ...
  const save = async () => {
    ...
    const students = rows.map(r => {
      ...
      return { name, admissionNumber, email, courseId, departmentId: deptId, institutionId };
    });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <motion.div initial={{ scale: 0.93, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
        className="w-full max-w-lg rounded-2xl border border-gray-200 bg-white border border-gray-200 p-6 shadow-2xl">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-semibold text-gray-800 flex items-center gap-2"><Upload className="h-4 w-4 text-sky-600" /> Bulk Import Students</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-700"><X className="h-5 w-5" /></button>
        </div>
        <p className="text-xs text-gray-400 mb-4">Paste CSV rows: <span className="font-mono text-gray-500">Name, AdmissionNumber, Email</span> (one per line)</p>
        <div className="space-y-3">
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button onClick={save} disabled={saving} className="flex-1 rounded-xl bg-sky-600 py-2.5 text-sm font-medium text-gray-900 hover:bg-sky-500 disabled:opacity-60">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-gray-800 flex items-center gap-2">
            <Users className="h-5 w-5 text-emerald-600" /> Students
          </h1>
          <p className="text-sm text-gray-400 mt-0.5">{students.length} registered · {filtered.length} shown</p>
        </div>
        <div className="flex gap-2">
          ...
          <button onClick={() => setShowAdd(true)}
            className="flex items-center gap-1.5 rounded-xl bg-emerald-600 px-3.5 py-2 text-sm font-medium text-gray-900 hover:bg-emerald-500">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200">
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-400 hidden md:table-cell">Department</th>
            ...
            <tbody>
              {loading ? (
                ...
                <motion.tr key={s.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }}
                  ...
                  <td className="px-4 py-3 text-gray-500 hidden md:table-cell">{s.department?.name ?? "—"}</td>
