...
import {
  ...
  AlertTriangle, CheckCircle2, AlertCircle, BookOpen, Activity, RefreshCw,
} from "lucide-react";
...
interface AnalyticsAlert { level: "critical" | "warning" | "info"; message: string; }
interface Building { building: string; total: number; active: number; rate: number; }

// ── Helpers (defined outside component to keep stable React identity) ──────
function SkeletonBar() {
  return <div className="h-2 rounded-full bg-gray-100 animate-pulse" />;
}
function SkeletonText({ w = "w-12" }: { w?: string }) {
  return <span className={`inline-block h-5 ${w} rounded bg-gray-200 animate-pulse`} />;
}
  ...
  if (loadError) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
        ...
        <div>
          <p className="text-sm font-semibold text-gray-700">Failed to load analytics</p>
          <p className="text-xs text-gray-400 mt-1">{loadError}</p>
        </div>
        <button onClick={fetchAll}
          className="flex items-center gap-2 rounded-lg border border-gray-200 px-4 py-2 text-sm text-gray-600 hover:bg-gray-50 transition-colors">
    ...
    <div className="space-y-6">
      ...
      {!loading && alerts.length > 0 && (
        <div className="space-y-2">
          ...
          {[...criticalAlerts, ...warningAlerts, ...infoAlerts].map((a, i) => {
            const Icon = ALERT_ICON_MAP[a.level] ?? AlertCircle;
            return (
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-5 lg:col-span-2">
          ...
          {loading ? (
            <div className="space-y-3">{[1,2,3,4].map(i => <SkeletonBar key={i} />)}</div>
          ) : deptHealth.length === 0 ? (
                        ...
                        <div className="flex items-center gap-2">
                          <div className="flex-1 h-1.5 rounded-full bg-gray-100">
                            <div style={{ width: `${d.attendance}%` }}
                              className={`h-full rounded-full ${d.attendance >= 75 ? "bg-emerald-500" : d.attendance >= 50 ? "bg-amber-400" : "bg-rose-400"}`} />
                          </div>
                        ...
                        <div className="flex items-center gap-2">
                          <div className="flex-1 h-1.5 rounded-full bg-gray-100">
                            <div style={{ width: `${d.submissionRate}%` }}
                              className={`h-full rounded-full ${d.submissionRate >= 75 ? "bg-indigo-500" : d.submissionRate >= 50 ? "bg-amber-400" : "bg-rose-400"}`} />
                          </div>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-5">
          ...
          {loading ? (
            <div className="space-y-4">{[1,2,3,4].map(i => <SkeletonBar key={i} />)}</div>
          ) : kpis ? (
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-5">
          ...
          {loading ? (
            <div className="space-y-3">{[1,2,3].map(i => <SkeletonBar key={i} />)}</div>
          ) : buildings.length === 0 ? (
