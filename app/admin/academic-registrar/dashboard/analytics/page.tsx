...
function HBar({ label, value, max, color, subtitle }: { label: string; value: number; max: number; color: string; subtitle?: string }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="w-32 shrink-0 text-right text-xs text-gray-500 truncate">{label}</div>
      <div className="flex-1 h-2 rounded-full bg-gray-200">
        <motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 0.7, ease: "easeOut" }}
          className={`h-full rounded-full ${color}`} />
      </div>
      <div className="w-12 shrink-0 text-xs text-gray-400 text-right">{subtitle ?? value}</div>
    </div>
  );
}
...
export default function FacultyAnalyticsPage() {
  ...
  const summary = [
    { icon: Building2,    label: "Departments",   value: departments.length, color: "text-sky-600",    bg: "bg-sky-500/10" },
    { icon: GraduationCap,label: "Lecturers",      value: lecturers.length,   color: "text-violet-600", bg: "bg-violet-500/10" },
    { icon: Users,        label: "Students",       value: students.length,    color: "text-emerald-600",bg: "bg-emerald-500/10" },
    { icon: Calendar,     label: "Weekly Sessions",value: entries.length,     color: "text-indigo-600", bg: "bg-indigo-500/10" },
    { icon: TrendingUp,   label: "Teaching Hrs/Wk",value: `${totalHours}h`,  color: "text-cyan-600",   bg: "bg-cyan-500/10" },
  ];
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-400 mt-0.5">Institution-wide metrics and resource overview</p>
      </div>
        ...
        {summary.map((s, i) => (
          <motion.div key={s.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
            ...
            <div className="text-xl font-bold text-gray-800">
              {loading ? <span className="h-5 w-8 bg-gray-200 rounded animate-pulse inline-block" /> : s.value}
            </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-gray-200 bg-white border border-gray-200 p-5">
          <div className="flex items-center gap-2 mb-5"><Users className="h-4 w-4 text-emerald-600" /><h2 className="text-sm font-semibold text-gray-900">Student Distribution by Department</h2></div>
          {loading ? <div className="space-y-2">{[1,2,3].map(i => <div key={i} className="h-5 rounded bg-gray-200 animate-pulse" />)}</div> : (
            <div className="space-y-3">
              {studentsPerDept.length === 0 ? <p className="text-sm text-gray-500 text-center py-4">No data</p>
        ...
        {/* Sessions per day */}
        <div className="rounded-2xl border border-gray-200 bg-white border border-gray-200 p-5">
          <div className="flex items-center gap-2 mb-5"><Calendar className="h-4 w-4 text-indigo-600" /><h2 className="text-sm font-semibold text-gray-900">Classes per Day of Week</h2></div>
          {loading ? <div className="space-y-2">{[1,2,3].map(i => <div key={i} className="h-5 rounded bg-gray-200 animate-pulse" />)}</div> : (
            <div className="space-y-3">
        ...
        {/* Sessions per dept */}
        <div className="rounded-2xl border border-gray-200 bg-white border border-gray-200 p-5">
          <div className="flex items-center gap-2 mb-5"><BarChart3 className="h-4 w-4 text-cyan-600" /><h2 className="text-sm font-semibold text-gray-900">Timetable Load per Department</h2></div>
          {loading ? <div className="space-y-2">{[1,2,3].map(i => <div key={i} className="h-5 rounded bg-gray-200 animate-pulse" />)}</div> : (
            <div className="space-y-3">
              {sessionsPerDept.length === 0 ? <p className="text-sm text-gray-500 text-center py-4">No data</p>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white border border-gray-200 p-5">
          <div className="flex items-center gap-2 mb-5"><TrendingUp className="h-4 w-4 text-sky-600" /><h2 className="text-sm font-semibold text-gray-900">Lecturer : Student Ratio per Dept</h2></div>
          {loading ? <div className="space-y-2">{[1,2,3].map(i => <div key={i} className="h-5 rounded bg-gray-200 animate-pulse" />)}</div> : (() => {
            ...
            return (
              <div className="space-y-3">
                {ratios.length === 0 ? <p className="text-sm text-gray-500 text-center py-4">No data</p>
