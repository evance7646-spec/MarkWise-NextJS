  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-indigo-100 bg-gradient-to-r from-indigo-50 to-sky-50 p-6 dark:border-indigo-500/30 dark:from-indigo-500/10 dark:to-sky-500/10">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <button
            type="button"
            onClick={exportData}
            className="rounded-xl border border-indigo-200 bg-indigo-50/70 px-4 py-2 text-sm font-medium text-indigo-700 transition-colors hover:bg-indigo-100 dark:border-indigo-600/40 dark:bg-indigo-500/10 dark:text-indigo-200 dark:hover:bg-indigo-500/20"
          >
            Export Data
          </button>
        </div>
      ...
      <div className="rounded-2xl border border-indigo-100 bg-white/90 p-6 dark:border-indigo-500/30 dark:bg-slate-950/80">
        ...
        {syncStatus && (
          <p className="mt-3 text-sm text-indigo-700 dark:text-indigo-700">{syncStatus}</p>
        )}

        {authError && (
          <div className="mt-3 rounded-lg border border-red-300 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-700 dark:bg-red-950/30 dark:text-red-700">
        ...
        {isLoading ? (
          <div className="mt-5 rounded-lg border border-black/10 p-6 text-sm text-zinc-600">
                  ...
                  <div className="flex w-full items-center justify-between gap-4 bg-gradient-to-r from-indigo-50 to-sky-50 px-4 py-4 text-left transition-colors hover:from-indigo-100 hover:to-sky-100 dark:from-indigo-500/10 dark:to-sky-500/10 dark:hover:from-indigo-500/20 dark:hover:to-sky-500/20">
                    ...
                    <div className="flex items-center gap-4 md:gap-6">
                      <button
                        ...
                        className="rounded-md border border-black/10 px-3 py-1 text-xs font-medium transition-colors hover:bg-black/5 dark:border-white/20 dark:hover:bg-gray-100"
                      >
                        Edit
                      </button>
                      <button
                        ...
                        className="rounded-md border border-red-300 px-3 py-1 text-xs font-medium text-red-600 transition-colors hover:bg-red-50 dark:border-red-500/50 dark:text-red-600 dark:hover:bg-red-500/10"
                      >
                        Delete
                      </button>
                      <button
                  ...
                  {isOpen && (
                    <div className="space-y-4 p-4">
                      {program.years.map((year) => {
                        ...
                        return (
                          <div key={year.id} className="rounded-xl border border-black/10 p-4">
                                    ...
                                    <div className="flex items-center justify-between gap-2">
                                      ...
                                      <div className="flex gap-2">
                                        <button
                                          ...
                                          className="rounded-md border border-black/10 px-2 py-1 text-xs font-medium transition-colors hover:bg-black/5 dark:border-white/20 dark:hover:bg-gray-100"
                                        >
                                          Add Unit
                                        </button>
                                        <button
                                      ...
                                      <table className="w-full text-left text-sm">
                                        ...
                                        <tbody>
                                          ...
                                          {semester.units.map((unit) => (
                                            <tr key={unit.id} className="border-t border-black/10">
                                              ...
                                              <td className="px-3 py-2">
                                                <div className="flex gap-2">
                                                  ...
                                                  <button
                                                    ...
                                                    className="rounded-md border border-red-300 px-2 py-1 text-xs font-medium text-red-600 transition-colors hover:bg-red-50 dark:border-red-500/50 dark:text-red-600 dark:hover:bg-red-500/10"
                                                  >
                                                    Delete
                                                  </button>
                                                </div>
...
function StatCard({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-xl border border-indigo-100 bg-gradient-to-br from-white to-indigo-50 p-4 dark:border-indigo-500/30 dark:from-slate-950 dark:to-indigo-500/10">
      <p className="text-xs uppercase tracking-wide text-indigo-600 dark:text-indigo-700">{label}</p>
  ...
  return (
    <button
      ...
      className={`rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
        destructive
          ? "border border-red-300 text-red-600 hover:bg-red-50 dark:border-red-500/50 dark:text-red-600 dark:hover:bg-red-500/10"
          : primary
            ? "bg-gradient-to-r from-indigo-600 to-sky-600 text-gray-900 hover:opacity-90"
            : "border border-indigo-200 bg-indigo-50/60 text-indigo-700 hover:bg-indigo-100 dark:border-indigo-600/40 dark:bg-indigo-500/10 dark:text-indigo-200 dark:hover:bg-indigo-500/20"
      }`}
    >
      {label}
    </button>
