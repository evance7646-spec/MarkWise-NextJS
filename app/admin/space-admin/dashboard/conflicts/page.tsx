    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-slate-100 flex items-center gap-2">
            <AlertTriangle className="h-5 w-5 text-rose-400" /> Booking Conflicts
          </h1>
          <p className="text-sm text-slate-500 mt-0.5">Double-booked rooms in the next 7 days</p>
        </div>
      ...
      {loading ? (
        <div className="space-y-3">
          {[1,2,3].map(i => <div key={i} className="h-36 rounded-2xl bg-slate-900 border border-slate-800 animate-pulse" />)}
        </div>
      ) : (
        <div className="space-y-3">
          {conflicts.map((c, i) => (
            <motion.div key={`${c.roomId}-${i}`}
              ...
              className="rounded-2xl border border-rose-500/20 bg-slate-900 overflow-hidden">
              {/* Room header */}
              <div className="flex items-center gap-3 px-5 py-3 bg-rose-500/5 border-b border-rose-500/10">
                ...
                <div className="flex-1 min-w-0">
                  <span className="font-medium text-rose-300">{c.roomName}</span>
                  <span className="ml-2 text-xs text-rose-400/60 font-mono">{c.buildingCode} · {c.roomCode}</span>
                </div>
                <span className="rounded-full bg-rose-500/20 px-2.5 py-0.5 text-xs font-medium text-rose-400">conflict</span>
              </div>

              {/* Two conflicting bookings */}
              <div className="grid sm:grid-cols-2 gap-px bg-slate-800/50">
                {[c.bookingA, c.bookingB].map((b, bi) => (
                  <div key={b.id} className="bg-slate-900 p-4">
                    <div className="text-xs text-slate-500 mb-1.5">Booking {bi === 0 ? "A" : "B"}</div>
                    <div className="flex items-center gap-1.5 text-sm text-slate-300 mb-0.5">
                      <Clock className="h-3.5 w-3.5 text-slate-500 shrink-0" />
                      {fmt(b.startAt)}
                    </div>
                    <div className="text-xs text-slate-500 pl-5 mb-1">
                      → {fmt(b.endAt)} ({durMins(b.startAt, b.endAt)} min)
                    </div>
                    {b.unitCode && (
                      <div className="text-xs font-mono text-amber-400 pl-5">{b.unitCode}</div>
                    )}
                    <div className="text-xs text-slate-600 font-mono pl-5 mt-1 truncate">{b.id}</div>
                    <button onClick={() => cancelBooking(b.id)} disabled={cancelling === b.id}
                      className="mt-3 rounded-xl bg-rose-500/10 px-3 py-1.5 text-xs font-medium text-rose-400 hover:bg-rose-500/20 disabled:opacity-50 transition-colors">
