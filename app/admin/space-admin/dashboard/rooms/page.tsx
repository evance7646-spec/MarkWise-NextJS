    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <motion.div initial={{ scale: 0.93, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
        ...
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-semibold text-slate-100 flex items-center gap-2"><Plus className="h-4 w-4 text-sky-400" /> Add Room</h2>
...
function StatusBadge({ room, onToggle }: { room: Room; onToggle: () => void }) {
  ...
  return (
    <button onClick={toggle} disabled={busy}
      className={`flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium transition-opacity ${room.isActive ? "bg-emerald-500/10 text-emerald-400" : "bg-slate-700 text-slate-500"} disabled:opacity-50`}
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-slate-100 flex items-center gap-2">
            <Building2 className="h-5 w-5 text-sky-400" /> Rooms
          </h1>
          <p className="text-sm text-slate-500 mt-0.5">{rooms.length} rooms · {filtered.length} shown</p>
        </div>
        <button onClick={() => setShowAdd(true)}
          className="flex items-center gap-1.5 rounded-xl bg-sky-600 px-3.5 py-2 text-sm font-medium text-white hover:bg-sky-500">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-800">
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 hidden md:table-cell">Building</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 hidden lg:table-cell">Type</th>
                <th className="text-center px-4 py-3 text-xs font-semibold text-slate-500 hidden lg:table-cell">Cap.</th>
              ...
              {loading ? (
                ...
                <tr><td colSpan={6} className="py-12 text-center">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-slate-500">{search ? "No rooms match your search" : "No rooms yet"}</p>
                  </div>
                </td></tr>
              ) : filtered.map((r, i) => (
                <motion.tr key={r.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }}
                  ...
                  <td className="px-4 py-3">
                    ...
                    <div className="text-xs text-slate-500 font-mono">{r.roomCode} · Fl {r.floor}</div>
                  </td>
                  <td className="px-4 py-3 text-slate-400 hidden md:table-cell font-mono text-xs">{r.buildingCode}</td>
                  <td className="px-4 py-3 hidden lg:table-cell">
                    <span className="rounded-full bg-slate-800 px-2 py-0.5 text-xs text-slate-400">{r.type}</span>
                  </td>
                  <td className="px-4 py-3 text-center text-slate-400 hidden lg:table-cell">{r.capacity}</td>
                  <td className="px-4 py-3 text-center">
                    <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium capitalize ${STATUS_COLOR[r.status] ?? "bg-slate-800 text-slate-400"}`}>
                      {r.status}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-center">
