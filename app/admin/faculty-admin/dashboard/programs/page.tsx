  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-slate-100">Programs &amp; Courses</h1>
          <p className="text-sm text-slate-500 mt-0.5">{programs.length} programs · {courses.length} courses institution-wide</p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => { setAddModal("program"); setForm(f => ({ ...f, name: "", durationYears: "3" })); setError(""); }}
            className="flex items-center gap-2 rounded-xl bg-amber-600 hover:bg-amber-500 px-3.5 py-2 text-xs font-semibold text-white transition-colors">
            <Plus className="h-3.5 w-3.5" /> Program
          </button>
          <button onClick={() => { setAddModal("course"); setForm(f => ({ ...f, name: "", code: "", programId: "" })); setError(""); }}
            className="flex items-center gap-2 rounded-xl bg-slate-700 hover:bg-slate-600 px-3.5 py-2 text-xs font-semibold text-slate-200 transition-colors">
      ...
      <div className="flex flex-wrap gap-2">
        <button onClick={() => setShowDeptId("all")} className={`rounded-xl px-3.5 py-1.5 text-xs font-medium transition-colors ${showDeptId === "all" ? "bg-amber-600 text-white" : "bg-slate-800 text-slate-400 hover:bg-slate-700"}`}>All</button>
        {departments.map(d => (
          <button key={d.id} onClick={() => setShowDeptId(d.id)} className={`rounded-xl px-3.5 py-1.5 text-xs font-medium transition-colors ${showDeptId === d.id ? "bg-amber-600 text-white" : "bg-slate-800 text-slate-400 hover:bg-slate-700"}`}>{d.name}</button>
        ))}
      </div>

      {loading ? (
        <div className="space-y-2">{[1,2,3].map(i => <div key={i} className="h-16 rounded-2xl bg-slate-900 animate-pulse" />)}</div>
      ) : filteredPrograms.length === 0 ? (
        <div className="flex flex-col items-center gap-2 py-12"><Layers className="h-8 w-8 text-slate-700" /><p className="text-sm text-slate-600">No programs found</p></div>
              ...
              {open && (
                <motion.div initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} className="overflow-hidden border-t border-slate-800">
                  {progCourses.length === 0 ? (
                    <p className="text-sm text-slate-600 px-4 py-4 text-center">No courses yet</p>
                  ) : (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
              ...
              <div className="flex items-center justify-between mb-5">
                <h2 className="text-base font-bold text-slate-100">Add {addModal === "program" ? "Program" : "Course"}</h2>
                <button onClick={() => setAddModal(null)} className="text-slate-500 hover:text-slate-300"><X className="h-5 w-5" /></button>
              </div>
              <div className="space-y-4">
                <div><label className={lbl}>Name <span className="text-red-400">*</span></label><input value={form.name} onChange={f("name")} className={inp} placeholder={addModal === "program" ? "e.g. Bachelor of Science" : "e.g. Algorithms"} /></div>
                {addModal === "program" && <>
                  ...
                  <div><label className={lbl}>Department <span className="text-red-400">*</span></label>
                ...
                {addModal === "course" && <>
                  <div><label className={lbl}>Code <span className="text-red-400">*</span></label><input value={form.code} onChange={f("code")} className={inp} placeholder="e.g. CS301" /></div>
                  <div><label className={lbl}>Program <span className="text-red-400">*</span></label>
              ...
              <div className="mt-5 flex justify-end gap-3">
                <button onClick={() => setAddModal(null)} className="rounded-xl border border-slate-700 px-4 py-2 text-sm text-slate-400 hover:bg-slate-800 transition-colors">Cancel</button>
                <button onClick={handleSave} disabled={saving} className="flex items-center gap-2 rounded-xl bg-amber-600 hover:bg-amber-500 disabled:opacity-50 px-4 py-2 text-sm font-semibold text-white transition-colors">
