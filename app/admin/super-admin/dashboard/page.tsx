    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-medium text-violet-400 uppercase tracking-wider">MarkWise Platform</span>
          </div>
          <h1 className="text-2xl font-bold text-white">
            Welcome, {admin?.fullName?.split(" ")[0] ?? "Admin"}
          </h1>
          <p className="text-sm text-slate-400 mt-0.5">Platform owner overview</p>
        </div>
        <button
          onClick={load}
          disabled={loading}
          className="flex items-center gap-2 rounded-xl border border-slate-700 bg-slate-800 px-3 py-2 text-sm text-slate-300 hover:bg-slate-700 transition-colors disabled:opacity-50"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {kpis.map(({ label, value, icon: Icon, color, bg, desc, href }, i) => (
          ...
          >
            <Link href={href}>
              <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5 hover:border-slate-600 transition-colors cursor-pointer">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
          <h2 className="text-sm font-semibold text-slate-300 mb-4">Platform Actions</h2>
          <div className="space-y-2">
            {[
              ...
              <Link key={href} href={href}>
                <div className="flex items-start gap-3 rounded-xl border border-slate-700 bg-slate-800 px-4 py-3 hover:bg-slate-700 hover:border-slate-600 transition-colors">
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold text-slate-300">Onboarded Institutions</h2>
            <Link
              href={`${BASE}/dashboard/institutions`}
          ...
          {loading ? (
            ...
            <div className="py-8 text-center">
              ...
              <p className="text-sm text-slate-500">No institutions onboarded yet.</p>
              <Link
                href={`${BASE}/dashboard/institutions`}
                className="mt-3 inline-flex items-center gap-1.5 rounded-xl bg-violet-500/15 px-3 py-1.5 text-xs font-medium text-violet-300 hover:bg-violet-500/25 transition-colors"
              >
            ...
            <div className="space-y-2">
              {institutions.map((inst) => (
                <div
                  key={inst.id}
                  className="flex items-center justify-between rounded-xl border border-slate-800 bg-slate-800/60 px-3 py-2.5"
                >
