...
import {
  LayoutDashboard, Building2, ShieldCheck, GraduationCap, Users,
  Layers, DoorOpen, BarChart3, Settings2,
  Menu, X, LogOut, ChevronRight, Landmark,
} from "lucide-react";
import { InstitutionAdminContext, type InstitutionAdminInfo } from "./context";

const BASE = "/admin/institution-admin";

const NAV = [
  { label: "Overview",     href: `${BASE}/dashboard`,                    icon: LayoutDashboard, color: "text-indigo-400" },
      ...
      <div className="border-t border-white/10 px-3 py-4">
        <button
          onClick={async () => {
            ...
            window.location.href = "/admin/institution-admin/login";
          }}
...
export default function InstitutionAdminLayout({ children }: { children: ReactNode }) {
  ...
  const [admin, setAdmin] = useState<InstitutionAdminInfo | null>(null);
  ...
  useEffect(() => {
    fetch("/api/auth/me", { credentials: "include" })
      ...
      .then(data => {
        if (!data) { router.push("/admin/institution-admin/login"); return; }
        setAdmin({
          id: data.id,
          fullName: data.fullName ?? "Admin",
          email: data.email ?? "",
          role: data.role ?? "institution_admin",
  ...
  return (
    <InstitutionAdminContext.Provider value={admin}>
      <div className="flex min-h-screen bg-slate-950 text-slate-100">
        {/* Desktop sidebar */}
        <aside className="hidden lg:flex w-60 shrink-0 flex-col bg-gradient-to-b from-indigo-950/60 to-slate-900 border-r border-white/8">
          ...
          {drawerOpen && (
            <>
              ...
              <motion.aside
                ...
                className="fixed inset-y-0 left-0 z-50 w-60 flex flex-col bg-gradient-to-b from-indigo-950/95 to-slate-900 border-r border-white/8 lg:hidden"
              >
