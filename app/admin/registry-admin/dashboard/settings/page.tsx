...
const inp = "w-full rounded-xl border border-slate-700 bg-slate-800 px-3 py-2.5 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500";
  ...
  return (
    <div className="space-y-6 max-w-xl">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5">Manage your registry admin account</p>
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div>
            ...
            <div className="rounded-xl border border-slate-700 bg-slate-800/50 px-3 py-2.5 text-sm text-slate-400">
              {admin?.institutionName ?? "—"}
            </div>
          </div>
          <div>
            <label className={lbl}>Institution ID</label>
            <div className="rounded-xl border border-slate-700 bg-slate-800/50 px-3 py-2.5 font-mono text-xs text-slate-500 truncate">
              {admin?.institutionId ?? "—"}
            </div>
          </div>
        </div>
      ...
      <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
        ...
        <div className="space-y-3">
          ...
          <button onClick={saveProfile} disabled={savingProfile}
            className="w-full rounded-xl bg-purple-600 py-2.5 text-sm font-medium text-white hover:bg-purple-500 disabled:opacity-60">
      ...
      <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
        className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
        <h2 className="text-sm font-semibold text-slate-300 mb-4 flex items-center gap-2">
          <Lock className="h-4 w-4 text-purple-400" /> Change Password
        </h2>
        <div className="space-y-3">
          ...
          {pwOk && <p className="text-xs text-emerald-400 flex items-center gap-1"><CheckCircle2 className="h-3.5 w-3.5" /> Password changed successfully</p>}
      ...
      <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
        ...
        <p className="text-xs text-slate-500 mb-4">You will be redirected to the registry login page.</p>
