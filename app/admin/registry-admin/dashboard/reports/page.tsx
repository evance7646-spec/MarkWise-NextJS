...
interface Student { id: string; name: string; admissionNumber: string; email: string; year: number; departmentId: string; courseId: string; course?: { name: string; code: string }; department?: { name: string } }
...
const REPORT_TYPES: { id: ReportType; label: string; icon: typeof Users; color: string; desc: string }[] = [
  { id: "students", label: "Student Roster", icon: Users, color: "text-emerald-400", desc: "Full list of registered students with contact info" },
  { id: "enrollment", label: "Enrollment Summary", icon: BookOpen, color: "text-sky-400", desc: "Course enrolment counts per department" },
  { id: "attendance", label: "Attendance Summary", icon: ClipboardCheck, color: "text-teal-400", desc: "Attendance rates per student for the institution" },
];
...
export default function ReportsPage() {
  ...
  const previewRows = (() => {
    if (reportType === "students") {
      return filteredStudents.slice(0, 10).map(s => [
        s.name, s.admissionNumber, s.email, `Year ${s.year}`,
  ...
  const HEADERS: Record<ReportType, string[]> = {
    students:   ["Name", "Admission No.", "Email", "Year", "Course", "Department"],
  ...
  const handleExport = () => {
    ...
    setTimeout(() => {
      ...
      if (reportType === "students") {
        rows = filteredStudents.map(s => [s.name, s.admissionNumber, s.email, `Year ${s.year}`, s.course?.name ?? "", s.department?.name ?? ""]);
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-slate-100 flex items-center gap-2">
            <BarChart3 className="h-5 w-5 text-cyan-400" /> Reports
          </h1>
          <p className="text-sm text-slate-500 mt-0.5">Generate and export registry reports as CSV</p>
        </div>
        <button onClick={handleExport} disabled={generating || loading}
          className="flex items-center gap-1.5 rounded-xl bg-cyan-600 px-3.5 py-2 text-sm font-medium text-white hover:bg-cyan-500 disabled:opacity-50">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        {REPORT_TYPES.map(r => (
          <button key={r.id} onClick={() => setReportType(r.id)}
            className={`rounded-2xl border p-4 text-left transition-colors ${reportType === r.id ? "border-cyan-500/40 bg-cyan-500/10" : "border-slate-800 bg-slate-900 hover:bg-slate-800/50"}`}>
            <r.icon className={`h-5 w-5 mb-2 ${r.color}`} />
            <div className="font-medium text-slate-200 text-sm">{r.label}</div>
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900 overflow-hidden">
        <div className="px-4 py-3 border-b border-slate-800 flex items-center justify-between">
          ...
          <span className="text-xs text-slate-600">{REPORT_TYPES.find(r => r.id === reportType)?.label}</span>
        </div>
              ...
              {loading ? (
                [...Array(5)].map((_, i) => (
                  <tr key={i} className="border-b border-slate-800/50">
                    {HEADERS[reportType].map((_, j) => <td key={j} className="px-4 py-3"><div className="h-4 rounded bg-slate-800 animate-pulse" /></td>)}
                  </tr>
                ))
                ...
                <tr><td colSpan={HEADERS[reportType].length} className="py-10 text-center">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-slate-500 text-xs">No data available for this report</p>
                  </div>
                </td></tr>
              ) : previewRows.map((row, i) => (
                <motion.tr key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                  className="border-b border-slate-800/40 hover:bg-slate-800/20">
                  {row.map((cell, j) => <td key={j} className="px-4 py-2.5 text-slate-300 text-xs">{cell}</td>)}
                </motion.tr>
              ))}
            </tbody>
          </table>
