  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-slate-100 flex items-center gap-2">
            <ClipboardCheck className="h-5 w-5 text-teal-400" /> Attendance Records
          </h1>
          <p className="text-sm text-slate-500 mt-0.5">Institution-wide attendance overview</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Total Records", value: records.length, color: "text-teal-400", bg: "bg-teal-500/10" },
          { label: "Present",       value: presentCount,   color: "text-emerald-400", bg: "bg-emerald-500/10" },
          { label: "Absent",        value: absentCount,    color: "text-rose-400", bg: "bg-rose-500/10" },
          { label: "Attendance Rate", value: `${rate}%`,   color: "text-sky-400", bg: "bg-sky-500/10" },
        ].map((s, i) => (
          <motion.div key={s.label} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
            className="rounded-2xl border border-slate-800 bg-slate-900 p-4">
            <div className={`text-2xl font-bold ${s.color}`}>
              {loading ? <span className="inline-block h-7 w-12 rounded bg-slate-800 animate-pulse" /> : s.value}
            </div>
      ...
      <div className="flex flex-wrap gap-2">
        <div className="relative flex-1 min-w-48">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)}
            ...
            className="w-full rounded-xl border border-slate-700 bg-slate-800 pl-9 pr-3 py-2.5 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-teal-500/50" />
        </div>
        {tab === "online" && (
              ...
              <tr className="border-b border-slate-800">
                ...
                {tab === "online"
                  ? <>
                      ...
                      <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 hidden lg:table-cell">Room</th>
                      <th className="text-center px-4 py-3 text-xs font-semibold text-slate-500">Status</th>
                    </>
                  : <>
                      <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 hidden md:table-cell">Unit</th>
                      <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500 hidden lg:table-cell">Session</th>
                      <th className="text-left px-4 py-3 text-xs font-semibold text-slate-500">Method</th>
                    </>
                }
              </tr>
            </thead>
              ...
              {loading ? (
                ...
                filteredOnline.length === 0 ? (
                  ...
                  <motion.tr key={r.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.01 }}
                    ...
                    <td className="px-4 py-3">
                      <div className="font-medium text-slate-200">{r.student?.name ?? "—"}</div>
                    ...
                    <td className="px-4 py-3 text-slate-500 text-xs hidden lg:table-cell">{r.room?.name ?? r.room?.roomNumber ?? "—"}</td>
                    <td className="px-4 py-3 text-center">
                      <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium capitalize ${STATUS_COLOR[r.status] ?? "text-slate-400 bg-slate-800"}`}>
                ...
                filteredOffline.length === 0 ? (
                  ...
                  <motion.tr key={r.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.01 }}
                    className="border-b border-slate-800/50 hover:bg-slate-800/30">
                    <td className="px-4 py-3">
                      <div className="font-medium text-slate-200">{r.student?.name ?? r.admissionNumber ?? "—"}</div>
                      <div className="text-xs text-slate-500">{r.admissionNumber}</div>
                    </td>
                    <td className="px-4 py-3 font-mono text-xs text-amber-400 hidden md:table-cell">{r.unitCode}</td>
                    <td className="px-4 py-3 text-xs text-slate-400 hidden lg:table-cell">
                      {new Date(r.sessionStart).toLocaleString("en-GB", { dateStyle: "medium", timeStyle: "short" })}
                    </td>
                    <td className="px-4 py-3">
                      <span className="rounded-full bg-teal-500/10 px-2 py-0.5 text-xs text-teal-400 capitalize">{r.method}</span>
                    </td>
                  </motion.tr>
                ))
              )}
