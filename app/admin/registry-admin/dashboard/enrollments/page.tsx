      ...
      <motion.div initial={{ scale: 0.93, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
        ...
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-semibold text-slate-100 flex items-center gap-2">
            <Plus className="h-4 w-4 text-sky-400" /> Enroll in Unit
          </h2>
          <button onClick={onClose} className="text-slate-500 hover:text-slate-300"><X className="h-5 w-5" /></button>
        </div>
        <p className="text-sm text-slate-400 mb-4">Student: <span className="text-sky-300 font-medium">{student.name}</span></p>
        <div className="space-y-3">
          ...
          {available.length === 0 && <p className="text-xs text-slate-500">All available units are already enrolled.</p>}
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button onClick={save} disabled={saving || available.length === 0}
              className="flex-1 rounded-xl bg-sky-600 py-2.5 text-sm font-medium text-white hover:bg-sky-500 disabled:opacity-60">
...
function StudentEnrollments({ student, allUnits, onChanged }: {
  ...
  return (
    <div className="pl-1 pt-2 space-y-1.5">
      {loading ? (
        <div className="text-xs text-slate-500 animate-pulse py-2">Loading enrollments…</div>
      ) : enrollments.length === 0 ? (
        <p className="text-xs text-slate-600 py-2">No units enrolled.</p>
      ) : enrollments.map(e => (
        <div key={e.id} className="flex items-center justify-between rounded-lg bg-slate-800/60 px-3 py-2">
          <div className="flex items-center gap-2">
            <BookOpen className="h-3.5 w-3.5 text-sky-400 shrink-0" />
            <span className="text-xs font-mono text-sky-300">{e.unit?.code}</span>
            <span className="text-xs text-slate-400 hidden sm:inline">— {e.unit?.title}</span>
          </div>
      ...
      <button onClick={() => setShowAdd(true)}
        className="flex items-center gap-1.5 text-xs text-sky-400 hover:text-sky-300 px-1 py-1">
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-slate-100 flex items-center gap-2">
            <ClipboardList className="h-5 w-5 text-sky-400" /> Enrollments
          </h1>
          <p className="text-sm text-slate-500 mt-0.5">Manage unit enrollments per student</p>
        </div>
      </div>
      ...
      <div className="relative">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)}
          ...
          className="w-full rounded-xl border border-slate-700 bg-slate-800 pl-9 pr-3 py-2.5 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-sky-500/50" />
      </div>

      <div className="space-y-2">
        {loading ? (
          [...Array(6)].map((_, i) => <div key={i} className="h-14 rounded-2xl bg-slate-900 border border-slate-800 animate-pulse" />)
        ) : filtered.length === 0 ? (
          <div className="flex flex-col items-center gap-2 py-10 rounded-2xl border border-slate-800 bg-slate-900">
            <AlertTriangle className="h-7 w-7 text-slate-700" />
            <p className="text-sm text-slate-500">{search ? "No students match your search" : "No students yet"}</p>
          </div>
        ) : filtered.map(s => {
          const open = expanded.has(s.id);
          return (
            <div key={s.id} className="rounded-2xl border border-slate-800 bg-slate-900 overflow-hidden">
              <button className="w-full flex items-center justify-between px-4 py-3.5 hover:bg-slate-800/40"
                onClick={() => toggle(s.id)}>
                <div className="flex items-center gap-3">
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-sky-500/15 text-sky-400 text-xs font-bold">
                    {s.name[0]?.toUpperCase()}
                  </div>
                  <div className="text-left">
                    <div className="text-sm font-medium text-slate-200 leading-tight">{s.name}</div>
                    <div className="text-xs text-slate-500">{s.admissionNumber} · {s.department?.name ?? ""} · {s.course?.name ?? ""}</div>
                  </div>
                </div>
              ...
              <AnimatePresence initial={false}>
                {open && (
                  <motion.div key="body" initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }}
                    ...
                    className="overflow-hidden border-t border-slate-800 px-4 pb-3">
                    <StudentEnrollments
