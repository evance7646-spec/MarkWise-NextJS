...
import { RegistryAdminContext, type RegistryAdminInfo } from "./context";

const BASE = "/admin/registry-admin";
      ...
      <div className="border-t border-white/10 px-3 py-4">
        <button
          onClick={async () => {
            ...
            window.location.href = "/registry/login";
          }}
...
export default function RegistryAdminLayout({ children }: { children: ReactNode }) {
  const pathname = usePathname();
  const router = useRouter();
  const [admin, setAdmin] = useState<RegistryAdminInfo | null>(null);
  ...
  useEffect(() => {
    fetch("/api/auth/me", { credentials: "include" })
      ...
      .then(data => {
        if (!data) { router.push("/registry/login"); return; }
        setAdmin({
          id: data.id,
          fullName: data.fullName ?? "Admin",
          email: data.email ?? "",
          role: data.role ?? "registry_admin",
          institutionId: data.institutionId ?? "",
          institutionName: data.institution?.name ?? "Registry",
        });
        setLoading(false);
      })
      .catch(() => router.push("/registry/login"));
  }, [router]);
  ...
  return (
    <RegistryAdminContext.Provider value={admin}>
      <div className="flex h-screen bg-slate-950 text-slate-100 overflow-hidden">
        {/* Desktop sidebar */}
        <aside className="hidden md:flex w-56 shrink-0 flex-col bg-gradient-to-b from-purple-950 to-slate-900 border-r border-white/8">
