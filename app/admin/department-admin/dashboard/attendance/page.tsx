...
import {
  ClipboardCheck, Search, AlertTriangle, CheckCircle2, XCircle,
} from "lucide-react";
...
export default function DeptAttendancePage() {
  ...
  const load = useCallback(async () => {
    ...
    const [att, off] = await Promise.all([
      fetch(`/api/attendance?departmentId=${admin.departmentId}`, { credentials: "include" }).then(r => r.ok ? r.json() : {}) as any,
      fetch(`/api/attendance/offline?departmentId=${admin.departmentId}`, { credentials: "include" }).then(r => r.ok ? r.json() : {}) as any,
    ]);
    setRecords(Array.isArray(att) ? att : (att.records ?? att.data ?? []));
    setOffline(Array.isArray(off) ? off : (off.records ?? off.data ?? []));
    setLoading(false);
  }, [admin?.departmentId]);
  ...
  const filteredOffline = offline.filter(r => {
    return !q || r.student?.name.toLowerCase().includes(q) || r.admissionNumber?.toLowerCase().includes(q);
  });
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">{admin?.departmentName} attendance overview</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Total Records",   value: records.length, color: "text-teal-600" },
          { label: "Present",         value: presentCount,   color: "text-emerald-600" },
          { label: "Absent",          value: absentCount,    color: "text-rose-600" },
          { label: "Attendance Rate", value: `${rate}%`,     color: "text-sky-600" },
        ].map((s, i) => (
        ...
        {(["online", "offline"] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`rounded-lg px-4 py-1.5 text-sm font-medium transition-colors capitalize
              ${tab === t ? "bg-slate-700 text-white shadow" : "text-gray-400 hover:text-gray-700"}`}>
              ...
              {loading ? (
                ...
                filteredOnline.length === 0 ? (
                  ...
                  <motion.tr key={r.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.01 }}
                    ...
                    <td className="px-4 py-3 text-gray-400 text-xs hidden lg:table-cell">
                      {r.room?.name ?? r.room?.roomNumber ?? "—"}
                    </td>
                    <td className="px-4 py-3 text-center">
                      <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium capitalize ${STATUS_COLOR[r.status] ?? "text-gray-500 bg-slate-100"}`}>
                ...
                filteredOffline.length === 0 ? (
                  <tr><td colSpan={4} className="py-12 text-center">
                    <div className="flex flex-col items-center gap-2">
                      <AlertTriangle className="h-7 w-7 text-slate-700" />
                      <p className="text-gray-400">No offline attendance records found</p>
                    </div>
                  </td></tr>
                  ...
                  <motion.tr key={r.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.01 }}
                    ...
                    <td className="px-4 py-3 font-mono text-xs text-teal-600 hidden md:table-cell">{r.unitCode}</td>
