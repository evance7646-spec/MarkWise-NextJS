    ...
    <div className="flex h-full flex-col">
      ...
      {admin && (
        <div className="px-4 py-3 border-b border-white/10">
          <p className="text-xs text-slate-400 truncate">{admin.departmentName}</p>
          <p className="text-[10px] text-slate-500 truncate">{admin.institutionName}</p>
        </div>
      )}
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-950">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-slate-400">Loading department portal…</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <DepartmentAdminContext.Provider value={admin}>
      <div className="flex h-screen bg-slate-950 text-slate-100 overflow-hidden">
        {/* Desktop sidebar */}
        <aside className="hidden md:flex w-56 shrink-0 flex-col bg-gradient-to-b from-teal-950 to-slate-900 border-r border-white/8 overflow-y-auto">
          ...
          {drawerOpen && (
            <>
              ...
              <motion.aside initial={{ x: -224 }} animate={{ x: 0 }} exit={{ x: -224 }} transition={{ type: "spring", damping: 30, stiffness: 300 }}
                className="fixed left-0 top-0 z-50 flex h-full w-56 flex-col bg-gradient-to-b from-teal-950 to-slate-900 md:hidden">
